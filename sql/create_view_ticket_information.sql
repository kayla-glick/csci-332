...
SELECT ticks.id AS ticket_id, ticks.account_id, ticks.purchase_date, shows.show_time, shows.price, thetrs.number as theater_number, cins.name as cinema_name, FullAddress(adds.id) AS address, movs.title AS movie_title, movs.genre, movs.rating, movs.run_time
FROM Tickets ticks
